    ...
    <div class="auth-inner">
      <div class="booking--wrapper">
        <div class="info--title">
          ...
          <h2 v-if="isTicketsLoaded">Ближайшие поездки</h2>
          <h2 v-else>Ближайшие поездки не найдены</h2>
          <div class="title--line"></div>
        </div>
        <div v-show="isTicketsLoaded" class="container"></div>
      </div>
    </div>
  </div>
  <div v-show="isTicketsLoaded" class="col">
      ...
      <div class="table-responsive">
        <table class="table tablesorter table align-items-center table-flush">
          ...
          >
            <th scope="row">
              <div class="media align-items-center">
                <div class="media-body">
                  <span class="name mb-0 text-sm">{{ ticket.date }}</span>
                </div>
              </div>
            </th>
            <td class="budget">
            ...
            <td>
              <div>
                {{ ticket.userDTO.firstName }}
                {{ ticket.userDTO.lastName }}
              </div>
            </td>
            <td>
              <div>
            ...
            <td>
                <span class="badge badge-warning badge-dot mr-4">
                  <i
                    v-if="ticket.ticketStatus == 'Забронирован'"
                    class="bg-info"
                  ></i>
                  <i
                    v-else-if="ticket.ticketStatus == 'Подтвержден'"
                    class="bg-success"
                  ></i>
                  <i v-else class="bg-danger"></i>
                  <span class="status"> {{ ticket.ticketStatus }}</span></span
                >
            </td>
            <td>
              <div>
